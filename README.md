...
OPTIONS:
    -f, --format <human|json|gitlab>   Output format (default: human)
        ...
        --library-version <2024-11|2025-02>
                                       Standard-library release the model targets (default: 2025-02).
                                       Declarative: reclassifies names retired since that release
                                       (e.g. Connections::FlowConnection) as W314 instead of E200.
...
stdlib.rs  → generated — every standard-library name and its package
spec.rs    → generated — the SysML v2 metaclass hierarchy
```

`src/spec.rs` and `src/stdlib.rs` are generated by `tools/gen-spec-table.py`
and `tools/gen-stdlib.py` from small indexes vendored in `spec/`, distilled
from the [OMG metamodel](https://www.omg.org/spec/SysML/20250201/SysML.json)
and the [standard library](https://github.com/Systems-Modeling/SysML-v2-Release).
They compile to static tables, so the binary stays dependency-free. Every rule
declares whether it comes from the specification, from the grammar, or is house
style — `--list-rules` prints that alongside each code.
...
cargo test                  # 160 unit tests (inline in src/*.rs) +
                             # 86 CLI integration tests (tests/cli.rs)
```
